...
with tab1:
    ...
    with get_connection() as conn:
        ...
        if st.button("Save Changes", key="save_tickers"):
            try:
                with get_connection() as save_conn:
                    save_conn.execute("DELETE FROM tickers")
                    for _, row in edited_df.iterrows():
                        save_conn.execute(
                            "INSERT INTO tickers (ticker, name, sector_id, market) VALUES (?, ?, ?, ?)",
...
with tab2:
    ...
    with get_connection() as conn:
        ...
        if st.button("Save Changes", key="save_sectors"):
            try:
                with get_connection() as save_conn:
                    save_conn.execute("DELETE FROM sectors")
                    for _, row in edited_df.iterrows():
                        save_conn.execute(
                            "INSERT INTO sectors (id, name_kr, name_en) VALUES (?, ?, ?)",
...
with tab3:
    ...
    with get_connection() as conn:
        ...
        if st.button("Save Changes", key="save_comps"):
            try:
                with get_connection() as save_conn:
                    save_conn.execute("DELETE FROM sector_competitors")
                    for _, row in edited_df.iterrows():
                        save_conn.execute(
                            "INSERT INTO sector_competitors (ticker, competitor_ticker) VALUES (?, ?)",
