...
- **A DCF intrinsic value below the current price is a SIGNAL, not an error.** Do NOT flag "intrinsic value < market price" (or "DCF and analyst target disagree") as a computational or consistency problem — that gap is the normal state for a stock the market prices on growth/expectations the conservative model deliberately excludes. A DCF is a *conservative model estimate with a stated range*, not a price prediction that must reconcile to the market. Only raise a DCF critique when the model MIS-USED its own inputs — e.g. the reported `discount_rate_pct`, `stage1_growth_pct`, `fcf_ttm`, or `terminal_growth_pct` are internally contradictory, or the analysis leads with the lone conservative `base` while ignoring the `fair_value`/`range_low`–`range_high` band the tool already provides. "The value should intuitively be higher" is NOT a valid critique; name the specific mis-used assumption or drop it.
