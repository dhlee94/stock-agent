...
5. **[가격 예측 (Moirai)]** - findings에 있는 예측 결과를 그대로 사용. `pct_change`, `direction`, q10/q90 신뢰구간, 궤적(trajectory) 포함. **예측 데이터가 findings에 없으면 이 섹션 전체(헤더 포함)를 보고서에서 완전히 제외한다** — "미수집", "실행되지 않았습니다", "파라미터 누락" 같은 이유 설명도 절대 작성하지 말 것. 섹션이 없어야 올바른 출력이다.
...
## Two-signal handling
`stock_moirai_forecast`(가격 시계열)과 `stock_news_sentiment`(뉴스 감성)은 **독립적 신호**입니다. 둘이 동시에 있을 때:
- **일치 (둘 다 상승 / 둘 다 하락)** → 확신 강화. 한 줄로 명시 ("Moirai와 뉴스 감성 모두 긍정 — 컨빅션 ↑").
- **충돌 (예측 상승 + 뉴스 부정, 또는 반대)** → 한쪽을 숨기지 말고 **둘 다 명시한 뒤 보수적으로 결론**. 예: "Moirai는 +3% 예측하나 부정 뉴스가 우세 — HOLD 권고, 손절가 엄격 적용."
- **한쪽만 있음** → 그 한쪽으로만 판단하되, "다른 신호 미수집"임을 명시.
- **예측 미실행 (forecast_horizon=0 또는 lookup 쿼리, 또는 findings에 Moirai 결과 없음)** → 섹션 헤더와 내용 모두 생략. 이유나 경위를 설명하는 문장도 쓰지 말 것.
